...
export default function RecipePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!recipe) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-20 text-center animate-fade-in-up">
        <h1 className="text-3xl font-bold text-stone-900">Recipe not found</h1>
        <Link href="/" className="text-orange-500 mt-4 inline-block font-semibold hover:underline">
          Return Home
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 w-full animate-fade-in-up">
      <Link href="/" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-stone-200 text-stone-600 hover:text-stone-900 shadow-sm hover:shadow transition-all duration-300 mb-8 group font-semibold">
      ...
      <div className="grid md:grid-cols-12 gap-12">
          ...
          <div className="bg-white rounded-[2rem] p-8 border border-stone-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] sticky top-28">
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-2xl bg-orange-50 flex items-center justify-center text-2xl shadow-inner text-orange-500">
                  ⏱️
                </div>
                <div>
              ...
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center text-2xl shadow-inner text-rose-500">
                  🍽️
                </div>
                <div>
              ...
              {recipe.chef?.[0] && (
                <div className="flex items-center gap-5 pt-8 border-t border-stone-100 mt-4">
                  <Link href={`/chefs/${recipe.chef[0].uid}`} className="w-16 h-16 rounded-full bg-stone-100 overflow-hidden shrink-0 shadow-inner group relative block border-2 border-transparent hover:border-orange-500 transition-all">
                    <img 
        ...
        <div className="md:col-span-8 space-y-12 mb-20">
          {(() => {
            const ingredientBlocks = recipe.content_sections
              ?.filter((block: any) => block.ingredients)
              .map((block: any) => block.ingredients) || [];
              
            const otherBlocks = recipe.content_sections
              ?.filter((block: any) => !block.ingredients) || [];
